...
function sentiment(text) {
  let cleanTextArray = text.replace(/[-,.?]/g, '').replace(/[\n]/g, ' ').split(' ').map(toLowerCase);
  let positive = countPositive(cleanTextArray);
  let negative = countNegative(cleanTextArray);
...
function countPositive(text) {
  let words = text.filter(word => positiveWords.includes(word));
  return words;
}

function countNegative(text) {
  let words = text.filter(word => negativeWords.includes(word));
  return words;
}
